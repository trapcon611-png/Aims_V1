...
interface Colors { roadColor: number; islandColor: number; background: number; shoulderLines: number; brokenLines: number; leftCars: number[]; rightCars: number[]; sticks: number; }
...
class CarLights {
  ...
  init() {
    ...
    for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
      ...
      aMetrics.push(radius); aMetrics.push(length); aMetrics.push(spd);
      aMetrics.push(radius); aMetrics.push(length); aMetrics.push(spd);
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // THIS IS THE FIX THAT STARTS THE MOVEMENT IMMEDIATELY
  ...
  update(delta: number) {
    ...
    // THIS IS THE SECOND FIX THAT ENSURES THE CLOCK TICKS
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    // Use full deep copy for options to prevent reference errors
    const options = { ...effectOptions } as HyperspeedOptions;
  ...
  return <div id="lights" ref={containerRef} style={{ width: '100%', height: '100%', position: 'absolute' }}></div>;
};
