...
import {
    ...
    Lock, Clock, Edit3, Unlock, CalendarDays, Banknote, Globe, FileSignature
} from 'lucide-react';
...
export default function AccountsPanel({ students }: { students: any[] }) {
    ...
    useEffect(() => {
        const checkVaultStatus = async () => {
            try {
                const status = await directorApi.getPoolStatus();
                if (status && status.isUnlocked !== undefined) {
                    setIsPoolUnlocked(status.isUnlocked);
                }
            } catch (e) { console.error("Vault check failed"); }
        };
        
        checkVaultStatus();
        const vaultInterval = setInterval(checkVaultStatus, 5000);
    ...
    const poolLedger = useMemo(() => {
        ...
        const online = txnsInRange.filter(t => ['ONLINE', 'NEFT'].includes(t.paymentMode));
        const cheque = txnsInRange.filter(t => t.paymentMode === 'CHEQUE');

        return {
            total: txnsInRange.reduce((sum, t) => sum + Number(t.amount), 0),
    ...
    const handleCheckboxToggle = (fieldKey: 'includeTuition' | 'includeDress' | 'includeBooks') => {
        setFeeForm(prev => {
            ...
            updated.amount = (updated.includeTuition ? Number(updated.tuitionFee) || 0 : 0) +
                        ...
                        <div className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    >
                                        <option>CASH</option><option>ONLINE</option><option>CHEQUE</option><option>NEFT</option>
                                    </select>
                                </div>
                                <div>
