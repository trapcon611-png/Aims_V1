...
export default function StudentCard({ child, onViewInvoice, isDark, token }: { child: ChildFinancial, onViewInvoice: (inv: any) => void, isDark: boolean, token: string }) {
  ...
  const getInstallmentStatus = (dueDateStr: string, index: number, instAmount: number) => { 
    ...
    const isPartiallyPaid = paid > cumulativeBefore && paid < (cumulativeBefore + instAmount);
    if (isPartiallyPaid) return { color: 'text-amber-500', bg: isDark ? 'bg-amber-900/20 border-amber-800' : 'bg-amber-50 border-amber-100', label: 'Partial', icon: Zap };
              ...
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                    <p className={`text-xs font-bold ${labelColor} uppercase tracking-widest mb-1`}>Total Agreed Fee</p>
                    <p className="text-2xl font-black">₹{child.totalFees.toLocaleString()}</p>
                  </div>
                  <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
                    <p className="text-xs font-bold text-emerald-500 uppercase tracking-widest mb-1">Paid So Far</p>
                    <p className="text-2xl font-black text-emerald-500">₹{child.paidFees.toLocaleString()}</p>
                  </div>
                </div>
              </div>
              ...
              {child.installments && child.installments.length > 0 && (
                <div className="space-y-3">
                  ...
                  {child.installments.map((inst, idx) => { 
                    ...
                    if (status.label === 'Partial') {
                        displayAmount = (cumulativeBefore + inst.amount) - child.paidFees;
                    }
                    ...
                    return ( 
                      <div key={idx} className={`flex justify-between items-center p-4 rounded-xl border transition-colors ${status.bg}`}>
                        ...
                        <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded ${isDark ? 'bg-slate-900 text-slate-300' : 'bg-white text-slate-600'} ${status.label === 'Partial' ? 'text-amber-600 bg-amber-100/50 border border-amber-200' : ''}`}>
                          {status.label}
                        </span>
                      </div> 
                    ); 
        ...
        {activeTab === 'invoices' && (
          <div className="space-y-4">
            {child.history.length === 0 ? (
              <div className={`p-12 text-center ${subTextColor} italic`}>No invoices found.</div>
            ) : (
              child.history.map((rec) => (
                <div key={rec.id} className={`flex justify-between items-center p-4 rounded-xl border transition-all hover:shadow-md ${isDark ? 'bg-slate-900 border-slate-800 hover:border-purple-800' : 'bg-white border-slate-200 hover:border-purple-200'}`}>
                  <div>
                    <p className="font-bold">₹{rec.amount.toLocaleString()}</p>
                    <p className={`text-xs ${subTextColor}`}>{new Date(rec.date).toLocaleDateString()} • {rec.paymentMode}</p>
                  </div>
                  <button onClick={() => onViewInvoice({ ...rec, studentName: child.name, studentId: child.studentId, parentId: child.parentId || 'N/A', batch: child.batch })} className={`p-2 rounded-lg transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-slate-400' : 'bg-slate-100 hover:bg-purple-50 text-slate-600 hover:text-purple-600'}`}>
                    <FileText size={18}/>
                  </button>
                </div>
              ))
        ...
        {activeTab === 'academics' && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                {loadingResults ? ( 
                  ...
                  <div className={`p-12 text-center ${subTextColor} italic border-2 border-dashed ${borderColor} rounded-xl`}>No exam results declared for this academic session yet.</div> 
                ) : ( 
                  <div className="space-y-4">
                    {results.map((res) => ( 
