...
const parentApi = {
  ...
  async getStudentResults(token: string, studentId: string) {
      try {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      particles.forEach((p, i) => {
        ...
        // Theme Particles
        ctx.fillStyle = isDark ? 'rgba(139, 92, 246, 0.5)' : 'rgba(147, 51, 234, 0.6)'; 
        ctx.fill();
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 160) { 
            ...
            ctx.strokeStyle = isDark 
                ? `rgba(139, 92, 246, ${0.15 * (1 - dist/160)})` 
        ...
        <div>
          ...
          <div className="flex justify-between items-center border-b-4 border-[#c1121f] pb-6 mb-8">
              ...
              <div className="text-right">
                 ...
                 <p className="text-sm text-slate-600 max-w-[200px] leading-tight mt-1">Royal Tranquil, 3rd Floor,<br/>Above Chitale Bandhu,<br/>Pimple Saudagar, Pune,<br/>MH 411027</p>
                      ...
                      <td className="py-4 px-4">
                          ...
                          {isCash ? (
                              ...
                              <p className="text-xs text-slate-500 italic mt-1">
                                {data.paymentMode} Ref/UPI ID: <span className="font-mono font-bold text-slate-700">{data.transactionId || 'N/A'}</span>
                              </p>
                          )}
        ...
        <div>
            <div className="mb-6 p-4 bg-slate-50 border border-slate-100 rounded text-[10px] text-slate-600 leading-relaxed text-justify">
                ...
                <ul className="list-disc pl-4 space-y-1">
                    ...
                    <li>If the fees are not paid by the due date, late payment fee of ₹1200/- will be charged for every month the payment remains overdue.</li>
                    <li>The institute reserves the right to suspend the student if the student is constantly absent and if fees are not paid after a specified time period.</li>
                    <li>Fees paid for the academic year or term are non-refundable except in special circumstances.</li>
                    <li>Parents will receive a payment confirmation after each transaction on WhatsApp or receipt copy.</li>
                    <li>If a student withdraws from the institute after the term has started, no refund will be provided based on the institute’s withdrawal policy.</li>
                    <li>Your registration will be cancelled without any notification in the following circumstances: Any misbehaviour with the teachers or institute staff.</li>
                </ul>
            </div>
...
const StudentCard = ({ child, onViewInvoice, isDark, token }: { child: ChildFinancial, onViewInvoice: (inv: any) => void, isDark: boolean, token: string }) => {
  ...
  // Load results when Academic tab is clicked
  useEffect(() => {
      if (activeTab === 'academics' && results.length === 0) {
          setLoadingResults(true);
          parentApi.getStudentResults(token, child.studentId)
            .then(data => {
  ...
  }, [activeTab, child.studentId, token, results.length]);
  ...
  return (
    <div className={`${cardBg} rounded-3xl shadow-xl border overflow-hidden transition-all duration-300 mb-8 flex flex-col lg:flex-row min-h-[500px]`}>
      ...
      <div className={`flex-1 p-6 lg:p-10 relative overflow-y-auto ${contentBg}`}>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-500 space-y-8">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}><p className={`text-xs font-bold ${labelColor} uppercase tracking-widest mb-1`}>Total Agreed Fee</p><p className="text-2xl font-black">₹{child.totalFees.toLocaleString()}</p></div>
                  <div className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}><p className="text-xs font-bold text-emerald-500 uppercase tracking-widest mb-1">Paid So Far</p><p className="text-2xl font-black text-emerald-500">₹{child.paidFees.toLocaleString()}</p></div>
                </div>
              </div>
                      ...
                      {child.installments.map((inst, idx) => { 
                          ...
                          return ( 
                              <div key={idx} className={`flex justify-between items-center p-4 rounded-xl border transition-colors ${status.bg}`}>
                                  ...
                                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded ${isDark ? 'bg-slate-900 text-slate-300' : 'bg-white text-slate-600'}`}>{status.label}</span>
                              </div> 
                          ); 
                      })}
                  </div>
        ...
        {activeTab === 'invoices' && (
          <div className="space-y-4">
              {child.history.length === 0 ? <div className={`p-12 text-center ${subTextColor} italic`}>No invoices found.</div> : child.history.map((rec) => (
                 <div key={rec.id} className={`flex justify-between items-center p-4 rounded-xl border transition-all hover:shadow-md ${isDark ? 'bg-slate-900 border-slate-800 hover:border-purple-800' : 'bg-white border-slate-200 hover:border-purple-200'}`}>
                    <div><p className="font-bold">₹{rec.amount.toLocaleString()}</p><p className={`text-xs ${subTextColor}`}>{new Date(rec.date).toLocaleDateString()} • {rec.paymentMode}</p></div>
                    <button onClick={() => onViewInvoice({ ...rec, studentName: child.name, studentId: child.studentId, parentId: child.parentId || 'N/A', batch: child.batch })} className={`p-2 rounded-lg transition ${isDark ? 'bg-slate-800 hover:bg-slate-700 text-slate-400' : 'bg-slate-100 hover:bg-purple-50 text-slate-600 hover:text-purple-600'}`}><FileText size={18}/></button>
                 </div>
              ))}
          </div>
        )}
        ...
        {activeTab === 'academics' && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500">
                {loadingResults ? (
                    <div className="flex flex-col items-center justify-center p-12 text-purple-500">
                        <Loader2 className="animate-spin mb-2" size={24}/>
                        <p className="text-xs font-bold uppercase tracking-widest">Fetching Report Cards...</p>
                    </div>
                ) : results.length === 0 ? (
                    <div className={`p-12 text-center ${subTextColor} italic border-2 border-dashed ${borderColor} rounded-xl`}>
                        No exam results declared for this academic session yet.
                    </div>
                ) : (
                    <div className="space-y-4">
                        {results.map((res) => (
                            <div key={res.id} className={`p-5 rounded-2xl border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} flex justify-between items-center`}>
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wider ${isDark ? 'bg-blue-900/30 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>{res.subject}</span>
                                        <span className={`text-xs ${subTextColor}`}>{new Date(res.date).toLocaleDateString()}</span>
                                    </div>
                                    <h4 className="font-bold text-lg">{res.title}</h4>
                                </div>
                                <div className="text-right flex items-center gap-6">
                                    <div>
                                        <p className={`text-[10px] font-bold uppercase tracking-wider ${labelColor}`}>Score</p>
                                        <p className="text-xl font-black">{res.score}<span className={`text-sm ${subTextColor}`}>/{res.totalMarks}</span></p>
                                    </div>
    ...
    <div className={`min-h-screen font-sans transition-colors duration-300 ${bgClass}`}>
      ...
      <header className={`${headerClass} backdrop-blur-md border-b px-6 py-4 flex justify-between items-center sticky top-0 z-50`}>
        ...
        <div className="flex items-center gap-4">
            ...
            <button onClick={onLogout} className={`p-2 rounded-lg transition ${isDark ? 'hover:bg-red-900/20 text-red-400' : 'hover:bg-red-50 text-red-500'}`}><LogOut size={20}/></button>
        </div>
      </header>
